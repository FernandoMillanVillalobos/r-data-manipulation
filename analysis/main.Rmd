...
The functions `complete`, `expand`, `nesting`, and `crossing` find combinations of values in vectors or columns. First, `complete` makes an existing data frame longer by converting implicit values to existing values. This means that combinations of values that are not present in the data are created as new rows. `complete` only works with the values that are already in the data. `expand`, in contrast, creates a new tibble. The tibble represents either all possible combinations of values or the unique combinations of values (with nesting). The `expand` function does something very similar to `complete`. However, instead of adding new rows with the complete set of values, a new data frame is created only for the columns you specify in the function. Suppose you have an incomplete tibble with months and years, in which the combination of the month “February” and the year 2013 is missing. You can use `expand` to create a complete set of years and months, including the combination of February and 2013. `complete` is actually a wrapper around `expand.` In other words, it is expand combined with `full_join`. With `complete` and `expand` we have completed data frames for missing rows. Sometimes, however, we are interested in the unique combinations of values in a data frame. This is basically the opposite of what we just did. Instead of finding all combinations of values we are looking for the unique combinations; in a given data frame! In this way the data shows what happened, not what didn’t happen.

`crossing` works similarly to `expand`, except that it uses vectors to create combinations of values in a data frame. However, as we will see later, `expand` can also do this. The difference from the other functions is that `crossing` does not need an existing data frame. `crossing` is usually a starting point for further analyses.

![Complete, expand, nesting and crossing functions](input/img/expand-df.png)
...
# creating new rows representing the runs (=race) in which it did not participate -> missing years
running_races_anna %>%
  complete(year, race)

# including all years present in the data (2012 and 2014 also)
running_races_anna %>%
  complete(year = 2010:2022, race)

# including more variables (="ultra marathons")
running_races_anna %>%
...
# including all years present in the data
running_races_anna %>%
  expand(year = 2010:2022, race)
# or
running_races_anna %>%
  expand(year = full_seq(year, 1), race) # 1 indicates that the years should be incremented by 1 each time

# adding the missing data (="minutes")
(all_running_races_anna <- running_races_anna %>%
  expand(year = full_seq(year, 1), race) %>%
  full_join(running_races_anna, by = c("year", "race")))

# complete() and expand() with group_by()
# getting the data
...
#  with group_by we expand the rows only within the runners (missing years by certain runners)
(all_running_races_club_correct <- running_races_club %>%
  group_by(runner) %>%
  expand(year = full_seq(year, 1), race = c(
    "10km", "half marathon",
    "marathon"
  )) %>%
  ungroup())

# getting the actual data of the three runners (=all years)
...
running_races_club %>%
  group_by(runner) %>%
  complete(year = full_seq(year, 1), race = c(
    "10km", "half marathon",
    "marathon"
  )) %>%
  ungroup()

# adding more analysis -> counting the number of races that a runner has or has not completed
(count_races <- complete_running_races_club %>%
  count(runner, race, missed_races = is.na(minutes)))

# completing row missing where the missed_races column is set to FALSE using fill()
count_races %>%
  complete(runner, race, missed_races, fill = list(n = 0)) # fill() allows us turn NAs to actual values.

# calculating the percentage of races the runners participated in
count_races %>%
  complete(runner, race, missed_races, fill = list(n = 0)) %>%
  pivot_wider(names_from = missed_races, values_from = n) %>%
  mutate(
    percent_races = round((`FALSE` / (`TRUE` + `FALSE`)) * 100, 1)
  )
...
# looking for the unique combinations in a data frame with expand() and nesting() -> what happened
running_races_club %>%
  expand(nesting(runner, race))

# finding out which runs the runners have never participated -> what didn´t happen
full_combinations_runners <- expand(running_races_club, runner,
...
# creating a data set with crossing()
crossing(
  year = 1905:2022,
  major = c(
    "Australien Open", "French Open",
    "Wimbledon", "US Open"
  )
)
